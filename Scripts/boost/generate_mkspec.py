...
def is_sequence(thing):
    return (isinstance(thing, collections.Sequence) and
            not isinstance(thing, basestring))


def shellescape(str_or_seq):
    if is_sequence(str_or_seq):
        return ' '.join(pipes.quote(arg) for arg in str_or_seq)
    else:
        return pipes.quote(str_or_seq)
